...
from std_msgs.msg import Float32, Int32, Int16, Float64
...
def calc_right(right_count):
    ...
    if right_count.data != 0.0 and last_encoder_right != 0.0:

        encoder_right = (right_count.data - last_encoder_right)

        if encoder_right > 10000.0:
            encoder_right = 0 - (3.4e+308 - encoder_right)
        elif encoder_right < -10000.0:
            encoder_right = 3.4e+308 - encoder_right
        else:
            d_right = encoder_right / xung_1m

    last_encoder_right = right_count.data

    print(f'right {encoder_right}')
...
def calc_left(left_count):
    ...
    if left_count.data != 0.0 and last_encoder_left != 0.0:

        encoder_left = (left_count.data - last_encoder_left)

        if encoder_left > 10000.0:
            encoder_left = 0 - (3.4e+308 - encoder_left)
        elif encoder_left < -10000.0:
            encoder_left = 3.4e+308 - encoder_left
        else:
            d_left = encoder_left / xung_1m

    last_encoder_left = left_count.data

    print(f'left {encoder_left}')
    print("")
...
if __name__ =="__main__":
    try:
        ...
        # rospy.Subscriber("left_ticks", Float64, calc_left )
        # rospy.Subscriber("right_ticks", Float64, calc_right)

        rospy.Subscriber("left_ticks", Float64, calc_left, queue_size=1, tcp_nodelay=True)
        rospy.Subscriber("right_ticks", Float64, calc_right, queue_size=1, tcp_nodelay=True)
