...
class OdomPublisher:
    def __init__(self):
        ...
        rospy.Subscriber("left_ticks", Float64, self.calc_left, queue_size=1)
        rospy.Subscriber("right_ticks", Float64, self.calc_right, queue_size=1)
    ...
    def calc_right(self, right_count):
        if right_count.data != 0 and self.last_encoder_right != 0:
            self.encoder_right = (right_count.data - self.last_encoder_right)

            if self.encoder_right > 10000:
                self.encoder_right = 0 - (65535 - self.encoder_right)
            elif self.encoder_right < -10000:
                self.encoder_right = 65535 - self.encoder_right
    ...
    def calc_left(self, left_count):
        if left_count.data != 0 and self.last_encoder_left != 0:
            self.encoder_left = (left_count.data - self.last_encoder_left)

            if self.encoder_left > 10000:
                self.encoder_left = 0 - (65535 - self.encoder_left)
            elif self.encoder_left < -10000:
                self.encoder_left = 65535 - self.encoder_left
    ...
    def update_odom(self):
        ...
        radian = math.asin((self.d_right - self.d_left) / self.R)
